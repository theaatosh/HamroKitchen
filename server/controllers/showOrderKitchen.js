      ...
      for (i = 0; i < orderCookIDDetails.length; i++) {
        if (orderCookIDDetails[i].kitchenId === userId) {
          for (j = 0; j < order.orderedItem.length; j++) {
            if (
              orderCookIDDetails[i].orderItemId.toString() ===
              order.orderedItem[j].id.toString()
            ) {
              orderItems.push({
                orderDetails: order,
...
const acceptOrder = async (req, res) => {
  ...
  try {
    ...
    if (allSameKitchen) {
      ...
      const arr = orders.orderCookIDDetails.filter(
        (obj) => obj.kitchenId.toString() === userId.toString()
      );
      const updated = await order.findByIdAndUpdate(orderId, {
        $set: {
...
const rejectOrder = async (req, res) => {
  ...
  try {
    const orders = await order.findById(orderId);
    const kitchenIds = orders.orderCookIDDetailsOriginal.map(
      (item) => item.kitchenId
    );
    const allSameKitchen = kitchenIds.every((id) => id === kitchenIds[0]);
    if (allSameKitchen) {
      ...
      const itemToMove = orders.orderCookIDDetails.filter(
        (item) => item.kitchenId.toString() === userId.toString()
      );
      ...
      if (
        updatedOrder.remaingOrderItemId.length ===
        updatedOrder.orderedItem.length
      ) {
        await order.findByIdAndUpdate(orderId, {
      ...
      for (let j = 0; j < orderCookIDDetails.length; j++) {
        if (orderCookIDDetails[j].kitchenId.toString() === userId.toString()) {
          for (let k = 0; k < order.orderedItem.length; k++) {
            if (
              orderCookIDDetails[j].orderItemId.toString() ===
              order.orderedItem[k].id.toString()
            ) {
              ...
              if (
                ...
                !order.partiallyAcceptedOrderID.some(
                  (partial) =>
                    partial.orderItemId.toString() ===
                      orderedItem.id.toString() &&
...
const completeOrder = async (req, res) => {
  ...
  try {
    ...
    const kitchenIds = orders.orderCookIDDetails.map((item) =>
      item.kitchenId.toString()
    );
    const allSameKitchen = kitchenIds.every((id) => id === kitchenIds[0]);
    ...
    if (allSameKitchen && kitchenIds.length === orders.orderedItem.length) {
      ...
      if (!itemToMove) {
        return res
          .status(400)
          .json({ message: "No item to complete for this kitchen" });
      }
      updated = await order.findByIdAndUpdate(orderId, {
        $set: {
    ...
    //  );
    if (
      updatedOrderData.partiallyCompletedOrderID.length ===
      updatedOrderData.orderedItem.length
    ) {
      await order.findByIdAndUpdate(orderId, {
      ...
      for (let j = 0; j < order.orderCookIDDetails.length; j++) {
        ...
        if (orderCookDetail.kitchenId.toString() === userId.toString()) {
          const orderedItem = order.orderedItem.find(
            (item) =>
              item.id.toString() === orderCookDetail.orderItemId.toString()
          );
          if (orderedItem) {
            orderItems.push({
