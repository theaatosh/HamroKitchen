...
const assignCookOrder = async (order) => {
  try {
    ...
    if (cook) {
      ...
      const orderItems = await orders.findById(orderId, {
        _id: 0,
        orderedItem: 1,
      });
      ...
      for (i = 0; i < cook.length; i++) {
        ...
        const cookFoodItem = await user.findById(cookId, {
          _id: 0,
          cookFoodItem: 1,
        });
        const cookFoodItemArray = Object.keys(cookFoodItem.cookFoodItem);
        console.log("cookFoodItemArray" + cookFoodItemArray);
        ...
        for (k = 0; k < orderItem.length; k++) {
          ...
          for (j = 0; j < cookFoodItemArray.length; j++) {
            if (
              orderItem[k].id ===
              cookFoodItem.cookFoodItem[cookFoodItemArray[j]]
            ) {
              console.log("here");
      ...
      if (orderItem.length === orderSperationToKitchen.length) {
        ...
        const orderItemIdMaking = orderSperationToKitchen.map(
          (item) => item.orderItemId
        );
        // console.log("here is the:"+orderItemIdMaking)
        // console.log(orderItem);
        const remaingOrderItemId = orderItem.filter(
          (item) => !orderItemIdMaking.includes(item.id)
        );
      ...
      if (orderItem.length === orderSperationToKitchen.length) {
        try {
          await orders.findByIdAndUpdate(orderId, {
            $set: {
              orderCookIDDetails: [
                ...(order.orderCookIDDetails || []),
                ...orderSperationToKitchen,
              ],
...
const fetchOrders = async () => {
  ...
  const order = await orders.find(
    {
      orderStatus: { $in: ["assignedToCookPartially", "processedWithPayment"] },
      paymentStatus: "paid",
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      if (scheduledTime <= currentDate) {
        ...
        if (order[i].orderStatus === "processedWithPayment") {
          ...
          order[i].orderStatus === "assignedToCookPartially" &&
